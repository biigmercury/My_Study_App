...
$$\frac{dy}{dx} + P(x)y = q(x)y^n \tag{7.1}$$
...
$$y^{-n}\frac{dy}{dx} + P(x)y^{1-n} = q(x) \tag{7.2}$$
...
$$\frac{dv}{dx} + (1-n)P(x)v = (1-n)q(x) \tag{7.3}$$
...
$$\frac{dy}{dx} = p(x)y^2 + q(x)y + r(x) \tag{Riccati}$$
...
$$\frac{dv}{dx} + (2p(x)y_1 + q(x))v = -p(x) \tag{7.4}$$
...
flowchart TD
    ...
    B -->|dy/dx + Py = qy^n| C[Bernoulli]
    B -->|dy/dx = py² + qy + r| D[Riccati]
    B -->|F(x, y', y'') = 0 no y term| E[Reduction of Order]
    C --> C1[Divide by y^n; set v = y^(1-n); linear ODE]
    D --> D1[Find one particular solution y1 by inspection]
    D1 --> D2[Substitute y = y1 + 1/v; linear ODE]
    E --> E1[Set p = y'; first-order ODE in p and x]
    E1 --> E2[Solve for p; integrate to get y]
```
