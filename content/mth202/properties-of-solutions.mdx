...
$$C_n(x, y) = x_n y_{n+1} - x_{n+1} y_n \tag{definition}$$
...
```mermaid
flowchart TD
    A[Auxiliary equation λ² + aλ + b = 0] --> B{Discriminant a² - 4b}
    B -->|a² - 4b > 0| C[Case I: Two distinct real roots λ₁ ≠ λ₂]
    B -->|a² - 4b = 0| D[Case II: Repeated real root λ = -a/2]
    B -->|a² - 4b < 0| E[Case III: Complex conjugate roots]
    C --> CF1[yₙ = c₁λ₁ⁿ + c₂λ₂ⁿ]
    D --> CF2[yₙ = (c₁ + c₂n)λⁿ]
    E --> CF3[yₙ = rⁿ(c₁ cos nθ + c₂ sin nθ)]
```
...
$$y_n = c_1 \lambda_1^n + c_2 \lambda_2^n \tag{14.4}$$
...
$$y_n = c_1 \cdot 3^n + c_2 \cdot (-2)^n \tag{14.7}$$
...
$$y_n = (c_1 + c_2 n)\lambda^n, \qquad \lambda = -\frac{a}{2} \tag{14.5}$$
...
$$y_n = r^n(c_1 \cos n\theta + c_2 \sin n\theta) \tag{14.6}$$
