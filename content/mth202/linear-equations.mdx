...
$$u\frac{dy}{dx} + uP(x)y = uQ(x) \tag{5.2}$$
...
$$\frac{du}{dx} = uP(x) \tag{variable separable!}$$

Separating and integrating: $\ln u = \int P(x)\,dx$, so:

$$u(x) = e^{\int P(x)\,dx} \tag{5.4}$$
...
$$\frac{d}{dx}[u(x)\,y] = u(x)\,Q(x) \tag{5.5}$$

Integrate both sides:

$$u(x)\,y = \int u(x)\,Q(x)\,dx + A \tag{5.6}$$
...
```mermaid
flowchart TD
    A[ODE: dy/dx + Py = Q] --> B[Find integrating factor u = e^∫Pdx]
    B --> C[Multiply: d/dx(uy) = uQ]
    C --> D[Integrate: uy = ∫uQ dx + A]
    D --> E[Divide by u: solve for y]
