...
$$y'' - 6y' + 5y = e^{3x} \tag{10.1}$$

subject to:

$$y(0) = 0, \qquad y'(0) = 1 \tag{10.2}$$
...
$$y = Ae^x + Be^{5x} - \frac{1}{4}e^{3x} \tag{10.3}$$

**Step 4. Apply Initial Conditions.**

From $y(0) = 0$:

$$A + B - \frac{1}{4} = 0 \implies A + B = \frac{1}{4} \tag{i}$$
...
$$A + 5B - \frac{3}{4} = 1 \implies A + 5B = \frac{7}{4} \tag{ii}$$
...
$$y = (A + Bx)e^{3x} + \frac{1}{2}x^2 e^{3x} \tag{10.3}$$
...
```mermaid
flowchart TD
    A[General Solution y = Ay₁ + By₂ + yₚ] --> B{Type of Conditions}
    B -->|Initial: y(x₀)=α and y'(x₀)=β same point| C[IVP]
    B -->|Boundary: y(x₀)=α and y(x₁)=β different points| D[BVP]
    C --> E[Substitute x=x₀ into y → Equation 1]
    C --> F[Substitute x=x₀ into y' → Equation 2]
    E --> G[Solve 2 equations simultaneously for A and B]
    F --> G
    G --> H[Unique particular solution always exists for well-posed IVP]
    D --> I[Substitute x=x₀ and x=x₁ → 2 equations in A and B]
