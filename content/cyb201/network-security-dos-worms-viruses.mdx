...
**Man-in-the-Mobile (MitMo)** is a variation of the Man-in-the-Middle attack that takes control of a **mobile device**. Once the phone is infected, it can be instructed to **exfiltrate the user's sensitive information** — quietly copying it and sending it to the attackers.

**Example:** **ZeuS** is a well-known piece of malware that used this technique. It could silently capture the **2-step verification (2FA) SMS codes** sent to a user — letting attackers defeat the very protection meant to keep accounts safe.

### Symptoms of Malware

Whatever its type, an infected system usually shows tell-tale signs. Recognising them early is the first step to responding before more damage is done:

- **Increased CPU usage** — malware running in the background consumes processing power.
- **Decreased computer speed** — the system feels sluggish as resources are stolen.
- **Frequent freezes or crashes** — the computer hangs or shuts down unexpectedly.
- **Decreased web browsing speed** — traffic may be intercepted or redirected.
- **Unexplained network connection problems** — malware communicating with attackers disrupts normal connectivity.
- **Files modified or deleted** — data the user never changed is altered or disappears.
- **Unknown files, programs, desktop icons, or processes appearing** — software the user never installed shows up.
- **Programs turning off or reconfiguring themselves** — especially security tools being silently disabled.
- **Email being sent without the user's knowledge or consent** — the device is being used to spread the infection or send spam.
