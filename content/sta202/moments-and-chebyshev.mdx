...
The **$r$-th raw moment** of $X$ is the expected value of $X^r$:
...
The term "about the origin" means we raise $X$ to a power without first subtracting the mean.
...
The **$r$-th central moment** measures the expected value of $(X - \mu)^r$ — we measure deviations from the mean, not from the origin:
